...
test('[ms-bing-speech-service] method _sendToSocketServer', function (t) {
  ...
  recognizer.start().then(() => {
    ...
    t.throws(recognizer._sendToSocketServer.bind(recognizer),
      new Error('could not send: connection to service not open'), 'service throws if socket is not connected');
  }).catch(console.error);
});
